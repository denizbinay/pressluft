...
2. **Run pre-commit validation**:
   - Execute `make check` from repo root and report any issues
   - If `make check` fails, run stepwise to isolate failures: `make lint`, `make test`, `make build`
   - For package-manager commands, run them under `web/` (for example: `pnpm --dir web lint`, `pnpm --dir web build`)
   - Never run root-level `pnpm` in this repo (no root `package.json`)
   - If validation fails, ask user if they want to proceed anyway or fix issues first
