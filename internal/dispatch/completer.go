...
func (c *Completer) HandleResult(result ws.CommandResult) error {
	...
	if err != nil {
		c.logger.Error("command result job lookup failed", observability.Correlation{ServerID: result.ServerID, CommandID: result.CommandID}.LogArgs("error", err)...)
	...
	if result.ServerID == "" {
		result.ServerID = ws.FormatAppID(job.ServerID)
	}
	corr := observability.Correlation{ServerID: job.ServerID, CommandID: result.CommandID}
...
func (c *Completer) HandleLogEntry(entry ws.LogEntry) error {
	...
	if err != nil {
		c.logger.Error("command log job lookup failed", observability.Correlation{ServerID: entry.ServerID, CommandID: entry.CommandID}.LogArgs("error", err)...)
		return err
	}
	if entry.JobID == "" {
		entry.JobID = job.ID
	...
	if entry.ServerID == "" {
		entry.ServerID = ws.FormatAppID(job.ServerID)
	}
	corr := observability.Correlation{ServerID: job.ServerID, CommandID: entry.CommandID}
...
func (c *Completer) emitTerminalActivity(ctx context.Context, job orchestrator.Job, message string, success bool) {
	...
	input.Payload = observability.Correlation{ServerID: job.ServerID, CommandID: derefCommandID(job.CommandID)}.Payload(map[string]any{"success": success})
	activityEntry, err := c.activity.Emit(ctx, input)
	if err != nil {
		c.logger.Error("command completion activity emit failed", observability.Correlation{ServerID: job.ServerID, CommandID: derefCommandID(job.CommandID)}.LogArgs("error", err)...)
		return
	}
	c.logger.Info("command completion activity emitted", observability.Correlation{ServerID: job.ServerID, CommandID: derefCommandID(job.CommandID)}.LogArgs("activity_id", activityEntry.ID, "success", success)...)
}
