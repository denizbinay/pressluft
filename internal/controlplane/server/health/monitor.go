...
func NewSiteHealthMonitor(siteStore *stores.SiteStore, domainStore *stores.DomainStore, activityStore *activity.Store, hub *ws.Hub, logger *slog.Logger) *SiteHealthMonitor {
	if logger == nil {
		logger = slog.Default()
	}
...
func (m *SiteHealthMonitor) reconcileAll(ctx context.Context) {
	...
	for _, site := range sites {
		if site.DeploymentState != stores.SiteDeploymentStateReady {
			continue
		}
...
func (m *SiteHealthMonitor) reconcileSite(ctx context.Context, site stores.StoredSite) {
	now := time.Now().UTC()
	routingState := stores.DomainRoutingStateReady
	routingMessage := "Hostname routing verified over HTTPS."
	runtimeState := stores.SiteRuntimeHealthStateHealthy
	runtimeMessage := "Public runtime checks passed."

	if err := VerifyPublicSiteRouting(ctx, site.ID, site.PrimaryDomain); err != nil {
		routingState = stores.DomainRoutingStateIssue
		routingMessage = err.Error()
		runtimeState = stores.SiteRuntimeHealthStateIssue
	...
	if m.hub != nil {
		...
		if info.Connected {
			if snapshot, err := m.fetchSiteHealthSnapshot(ctx, site); err == nil {
				if state, message := RuntimeHealthFromAgentSnapshot(snapshot); state == stores.SiteRuntimeHealthStateIssue {
					runtimeState = state
					runtimeMessage = "Agent diagnostics: " + message
				} else if runtimeState == stores.SiteRuntimeHealthStateHealthy {
					runtimeMessage = "Public and managed-server runtime checks passed."
				}
			} else if runtimeState == stores.SiteRuntimeHealthStateHealthy {
				runtimeState = stores.SiteRuntimeHealthStateUnknown
...
func (m *SiteHealthMonitor) fetchSiteHealthSnapshot(ctx context.Context, site stores.StoredSite) (*agentcommand.SiteHealthSnapshot, error) {
...
func (m *SiteHealthMonitor) primaryDomainForSite(ctx context.Context, siteID string) (*stores.StoredDomain, error) {
	if m.domainStore == nil {
...
func (m *SiteHealthMonitor) emitTransition(ctx context.Context, site stores.StoredSite, runtimeState, runtimeMessage string) {
	...
	if runtimeState == stores.SiteRuntimeHealthStateIssue {
...
func siteWordPressRootPath(site stores.StoredSite) string {
