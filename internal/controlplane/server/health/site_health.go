...
func RuntimeHealthFromAgentSnapshot(snapshot *agentcommand.SiteHealthSnapshot) (string, string) {
	if snapshot == nil {
		return stores.SiteRuntimeHealthStateUnknown, "Managed server diagnostics are unavailable."
	}
	if snapshot.Healthy {
		return stores.SiteRuntimeHealthStateHealthy, strings.TrimSpace(snapshot.Summary)
	}
	...
	return stores.SiteRuntimeHealthStateIssue, message
}
