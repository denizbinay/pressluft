...
const { activities, loading, error, listServerActivity, streamActivity } =
  useActivity();
...
const serverFilter = (activity: Activity) => {
  const isResource =
    activity.resource_type === "server" &&
    activity.resource_id === props.serverId;
  const isParent =
  ...
  <div class="space-y-4">
    ...
    >
      ...
      <Button
        ...
        :class="
          cn(
            'mt-2 h-auto px-0 text-xs text-destructive hover:text-destructive/80',
          )
        "
    ...
    <div v-else class="space-y-3">
      ...
      >
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              :class="
                cn(badgeVariantClass(levelVariant(activity.level)), 'text-xs')
              "
            >
              {{ activity.level }}
            </Badge>
          ...
          <p class="mt-0.5 text-xs text-muted-foreground">
            <span v-if="activityMeta(activity)"
        ...
        <Button
          ...
          size="sm"
          :class="
            cn('h-auto shrink-0 px-0 text-xs text-accent hover:text-accent/80')
          "
        >
