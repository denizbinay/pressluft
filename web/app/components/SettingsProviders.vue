...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <UiButton size="sm" @click="openAddModal">
    ...
    <div v-else-if="providers.length === 0" class="rounded-lg border border-dashed border-surface-700/50 px-4 py-12 text-center">
      ...
      <div class="mt-4">
        <UiButton size="sm" variant="outline" @click="openAddModal">
    ...
    <div v-else class="space-y-3">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <UiBadge :variant="p.status === 'active' ? 'success' : 'default'">
                {{ p.status }}
              </UiBadge>
            </div>
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-surface-400 hover:text-danger-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <UiModal :open="modal.isOpen.value" title="Add Cloud Provider" @close="modal.close">
      <div class="space-y-5">
        ...
        <template v-if="formStep === 'configure'">
          ...
          <div class="space-y-1.5">
            <label class="block text-sm font-medium text-surface-300">Provider</label>
          ...
          >
            <div v-if="formValidation" :class="[
              'rounded-lg border px-3.5 py-2.5 text-sm',
              formValidation.valid && formValidation.read_write
        ...
        <template v-if="formStep === 'validated'">
          <!-- Success banner -->
          <div class="rounded-lg border border-success-600/30 bg-success-900/20 px-3.5 py-2.5 text-sm text-success-400">
            <div class="flex items-center gap-2">
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-2">
          <UiButton variant="ghost" size="sm" @click="modal.close">
            Cancel
          </UiButton>
