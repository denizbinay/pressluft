...
<template>
  <div class="min-h-screen bg-surface-950 text-surface-50">
    <div class="flex min-h-screen">
      <div
        v-if="isSidebarOpen"
        class="fixed inset-0 z-40 bg-surface-950/70 backdrop-blur-sm lg:hidden"
        @click="closeSidebar"
      />
      ...
      <aside
        ...
        :class="[
          'fixed inset-y-0 left-0 z-50 flex h-full flex-col border-r border-surface-800/60 bg-surface-950/95 backdrop-blur transition-transform duration-200 lg:static lg:translate-x-0',
          isSidebarCollapsed ? 'w-20' : 'w-72',
          isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
          !isSidebarCollapsed && 'lg:resize-x',
        ]"
      >
        <div class="flex items-center justify-between px-4 py-4">
          <NuxtLink to="/" class="flex items-center gap-2">
            ...
            <span v-if="!isSidebarCollapsed" class="text-lg font-semibold tracking-tight text-white">
              Pressluft
            </span>
          </NuxtLink>
          ...
          <button
            type="button"
            class="inline-flex h-8 w-8 items-center justify-center rounded-md text-surface-400 transition hover:bg-surface-900/70 hover:text-surface-100 lg:hidden"
        ...
        <nav class="flex-1 px-3 pb-6">
          <div v-for="section in navSections" :key="section.title" class="space-y-3">
            <p
              ...
              class="px-3 text-[10px] font-medium uppercase tracking-wider text-surface-500"
            >
              {{ section.title }}
            </p>
            <div class="space-y-1">
        ...
        <div class="border-t border-surface-800/60">
          <UserPanel :collapsed="isSidebarCollapsed" />
        </div>
      </aside>
