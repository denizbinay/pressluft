...
const buildActivityParams = (
  ...
  if (typeof filter.requiresAttention === "boolean") {
    params.set(
      "requires_attention",
      filter.requiresAttention ? "true" : "false",
    );
  }
...
export function useActivity() {
  ...
  const listActivity = async (
    ...
    try {
      ...
      const payload = parseActivityListResponse(
        await apiFetch(`/activity${query ? `?${query}` : ""}`),
      );
  ...
  const streamActivity = (options: ActivityStreamOptions = {}) => {
    ...
    const connect = () => {
      if (closed) return;
      try {
        const query = lastSeenId ? `?since_id=${lastSeenId}` : "";
        ...
        stream.addEventListener("activity", (evt) => {
          try {
            ...
            if (!lastSeenId || parsed.id > lastSeenId) {
            ...
            if (
              options.maxItems &&
              activities.value.length > options.maxItems
            ) {
  ...
  const markRead = async (activityId: string) => {
    const payload = parseActivity(
      await apiFetch(`/activity/${activityId}/read`, {
        method: "POST",
      }),
    );
  ...
  const markAllRead = async (filter: ActivityFilter = {}) => {
    const params = buildActivityParams(filter);
    const query = params.toString();
    await apiFetch(`/activity/read-all${query ? `?${query}` : ""}`, {
      method: "POST",
    });
    ...
    const matchesFilter = (activity: Activity) => {
      if (filter.category && activity.category !== filter.category)
        return false;
      if (filter.resourceType && activity.resource_type !== filter.resourceType)
        return false;
      if (filter.resourceId && activity.resource_id !== filter.resourceId)
        return false;
      if (
        filter.parentResourceType &&
        activity.parent_resource_type !== filter.parentResourceType
      ) {
        return false;
      }
      if (
        filter.parentResourceId &&
        activity.parent_resource_id !== filter.parentResourceId
      ) {
        return false;
      }
      if (typeof filter.requiresAttention === "boolean") {
        if (activity.requires_attention !== filter.requiresAttention)
          return false;
      }
