...
site_slug="smoke-clone-$(date +%s)"
echo "[smoke] creating site slug=${site_slug}"
site_resp="$(curl -sS -b "${COOKIE_JAR}" -H "Content-Type: application/json" -d "{\"name\":\"Smoke Clone Site\",\"slug\":\"${site_slug}\"}" -w "\n%{http_code}" "${BASE_URL}/api/sites")"
...
site_job_status="$(json_get "${site_job_json}" "data['status']")"
if [[ "${site_job_status}" != "succeeded" ]]; then
...
source_env_id="$(python3 - <<'PY' "${envs_json}"
import json, sys
items = json.loads(sys.argv[1])
...
clone_slug="clone-$(date +%s)"
echo "[smoke] creating clone environment slug=${clone_slug}"
clone_resp="$(curl -fsS -b "${COOKIE_JAR}" -H "Content-Type: application/json" -d "{\"name\":\"Smoke Clone\",\"slug\":\"${clone_slug}\",\"type\":\"clone\",\"source_environment_id\":\"${source_env_id}\",\"promotion_preset\":\"content-protect\"}" "${BASE_URL}/api/sites/${site_id}/environments")" || fail "failed command: POST /api/sites/${site_id}/environments"
clone_job_id="$(json_get "${clone_resp}" "data['job_id']")"
clone_job_json="$(wait_job_terminal "${clone_job_id}")"
clone_job_status="$(json_get "${clone_job_json}" "data['status']")"
if [[ "${clone_job_status}" != "succeeded" ]]; then
  fail "clone create job failed"
fi

envs_json="$(curl -fsS -b "${COOKIE_JAR}" "${BASE_URL}/api/sites/${site_id}/environments")" || fail "failed command: GET /api/sites/${site_id}/environments"
clone_preview="$(python3 - <<'PY' "${envs_json}" "${clone_slug}"
import json, sys
items = json.loads(sys.argv[1])
slug = sys.argv[2]
env = next((e for e in items if e.get('slug') == slug), None)
if not env:
    raise SystemExit(1)
print(env['preview_url'])
PY
)" || fail "failed to resolve clone preview URL"

echo "[smoke] validating clone preview URL ${clone_preview}"
for _ in $(seq 1 20); do
  if curl_ok "${clone_preview}"; then
    echo "[smoke] success: clone preview URL reachable"
    exit 0
  fi
  sleep 3
done

fail "clone preview URL unreachable"
